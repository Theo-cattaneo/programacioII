...
class VisualTrackingApp:
    def __init__(self, root):
        ...
        self.flash_speed = 5
        # RE-INTRODUCIDO: Variable para el aumento de velocidad
    ...
    def limpiar_menu_actual(self):
        """Destruye el frame del menú actual si existe."""
    ...
    def move_flash(self):
        ...
        # --- LÓGICA COMBINADA: Detección de "colisión virtual" y aumento de velocidad ---
        collision_occurred = False
        if self.movement_mode == 'horizontal':
            # Detecta si el centro del objeto cruzó el borde derecho o izquierdo
            if self.vx > 0 and cx > canvas_width:
                collision_occurred = True
            elif self.vx < 0 and cx < 0:
                collision_occurred = True
        elif self.movement_mode == 'vertical':
            # Detecta si el centro del objeto cruzó el borde inferior o superior
            if self.vy > 0 and cy > canvas_height:
                collision_occurred = True
            elif self.vy < 0 and cy < 0:
                collision_occurred = True

        if collision_occurred:
            # Aumentar la velocidad manteniendo la dirección
        ...
        # --- Lógica de Movimiento Continuo (Wrap-Around) ---
        if self.movement_mode == 'horizontal':
            if cx > canvas_width + self.flash_size:
                self.canvas.move(self.flash_item, -canvas_width - 2 * self.flash_size, 0)
            elif cx < -self.flash_size:
                self.canvas.move(self.flash_item, canvas_width + 2 * self.flash_size, 0)
        
        elif self.movement_mode == 'vertical':
            if cy > canvas_height + self.flash_size:
                self.canvas.move(self.flash_item, 0, -canvas_height - 2 * self.flash_size)
            elif cy < -self.flash_size:
                self.canvas.move(self.flash_item, 0, canvas_height + 2 * self.flash_size)
